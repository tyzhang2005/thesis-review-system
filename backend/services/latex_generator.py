...
def generate_latex_report(doc, output: dict, output_file: str):
    # 将15部分评估结果填充到LaTeX模板中
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        f.write(r"""\documentclass[UTF8]{article}
    ...
    with open(output_file, "a", encoding="utf-8") as f:
        f.write(r"\newcommand{\studentid}{ ")
        f.write(doc.metadata.get("student_id", "N/A"))
        f.write(r"}  % 学号")
        f.write("\n")
        f.write(r"\newcommand{\studentname}{ ")
        f.write(doc.metadata.get("student_name", "N/A"))
        f.write(r"}  % 姓名")
        f.write("\n")
        f.write(r"\newcommand{\username}{ ")
        f.write(doc.metadata.get("student_name", "N/A"))
        ...
        f.write(doc.metadata.get("title", "N/A"))
        f.write("}  % 论文标题")
        f.write("\n\n")

    # 定义评分项映射
    score_mapping = [
        ("formatScoreOne", 0),  # 摘要评分
        ("formatScoreTwo", 1),  # 摘要关键词评分
        ("formatScoreThree", 2),  # 目录评分
        ("formatScoreFour", 3),  # 主体部分评分
        ("formatScoreFive", 4),  # 参考文献评分
        ("formatScoreSix", 5),  # 谢辞评分
        ("topicScoreOne", 6),  # 培养目标评分
        ("topicScoreTwo", 7),  # 培养方案评分
        ("topicScoreThree", 8),  # 选题价值评分
        ("levelScoreOne", 9),  # 文献检索评分
        ("levelScoreTwo", 10),  # 知识应用评分
        ("levelScoreThree", 11),  # 方法工具评分
        ("levelScoreFour", 12),  # 专业技能评分
        ("levelScoreFive", 13),
        ("levelScoreSix", 14),
        ("qualityScoreOne", 15),
        ("qualityScoreTwo", 16),
        ("qualityScoreThree", 17),
    ]

    # 处理并写入18个评分项
    for prefix, score_index in score_mapping:
        score = section_scores[score_index]

        with open(output_file, "a", encoding="utf-8") as f:
            # 根据分数确定打钩位置
            if score == 3:  # 优秀
                f.write(f"\\newcommand{{\\{prefix}A}}{{\\checkmark}}")
                f.write(f"\\newcommand{{\\{prefix}B}}{{}}")
                f.write(f"\\newcommand{{\\{prefix}C}}{{}}")
                f.write(f"\\newcommand{{\\{prefix}D}}{{}}\n")
            elif score == 2:  # 良好
                f.write(f"\\newcommand{{\\{prefix}A}}{{}}")
                f.write(f"\\newcommand{{\\{prefix}B}}{{\\checkmark}}")
                f.write(f"\\newcommand{{\\{prefix}C}}{{}}")
                f.write(f"\\newcommand{{\\{prefix}D}}{{}}\n")
            elif score == 1:  # 一般
                f.write(f"\\newcommand{{\\{prefix}A}}{{}}")
                f.write(f"\\newcommand{{\\{prefix}B}}{{}}")
                f.write(f"\\newcommand{{\\{prefix}C}}{{\\checkmark}}")
                f.write(f"\\newcommand{{\\{prefix}D}}{{}}\n")
            elif score == 0:  # 较差
                f.write(f"\\newcommand{{\\{prefix}A}}{{}}")
                f.write(f"\\newcommand{{\\{prefix}B}}{{}}")
                f.write(f"\\newcommand{{\\{prefix}C}}{{}}")
                f.write(f"\\newcommand{{\\{prefix}D}}{{\\checkmark}}\n")
            else:  # 未知分数
                f.write(f"\\newcommand{{\\{prefix}A}}{{}}")
                f.write(f"\\newcommand{{\\{prefix}B}}{{}}")
                f.write(f"\\newcommand{{\\{prefix}C}}{{}}")
    ...
    def format_suggestions_for_latex(suggestions: str) -> str:
        """将普通文本转换为LaTeX格式，处理换行"""
        if not suggestions:
            return ""

        import re

        # 将两个或更多个连续换行符替换为\par\par
        latex_suggestions = re.sub(r"\n+", r"\\par", suggestions)

        # 转义特殊字符
        latex_suggestions = latex_suggestions.replace("_", r"\_")
        latex_suggestions = latex_suggestions.replace("#", r"\#")
        latex_suggestions = latex_suggestions.replace("%", r"\%")
        latex_suggestions = latex_suggestions.replace("&", r"\&")

        return latex_suggestions

    # 写入总评分和修改建议
    with open(output_file, "a", encoding="utf-8") as f:
        f.write(r"\newcommand{\totalScore}{")
        f.write(str(section_scores[18]))  # 第18项是百分制分数
        f.write("}  % 总评分\n")

        f.write(r"\newcommand{\suggestions}{")
        formatted_suggestions = format_suggestions_for_latex(summary)
