...
# 静态演示用的数据目录
DISPLAY_DIR = DATA_DIR / "display_data"

# 静态演示用的路径配置 - 与teacher_review保持一致
STUDENT_RESULTS_PATH = DISPLAY_DIR / "student_results"
STUDENT_PDFS_PATH = DISPLAY_DIR / "student_pdfs"
ANNOTATIONS_PATH = DISPLAY_DIR / "teacher_annotations"
...
# 云端API配置
CLOUD_API_ENDPOINT = get_setting(
    "api.cloud.endpoint", "https://dashscope.aliyuncs.com/compatible-mode"
)
CLOUD_MODEL = get_setting("api.cloud.model", "deepseek-v3.2-exp")

# 云端嵌入模型配置
USE_CLOUD_EMBEDDING = get_setting("models.use_cloud_embedding", True)
CLOUD_EMBEDDING_ENDPOINT = get_setting(
    "api.cloud.embedding.endpoint",
...
VLLM_EMBEDDING_ENDPOINT = get_setting(
    "api.vllm.embedding_endpoint", "http://localhost:8004/v1/embeddings"
)
...
AIGC_MODEL_PATH = get_setting(
    "models.aigc_detector_path", "./models/AIGC_text_detector"
)
...
if not MINERU_TOKEN:
    logging.warning(
        "MINERU_TOKEN not set in environment variables. PDF conversion may not work."
    )
...
DATABASE_VECTORSTORE_DIR_LOCAL = DATA_DIR / get_setting(
    "vectorstore.local_dir", "databases/vectorstore_local"
)
DATABASE_VECTORSTORE_DIR_CLOUD = DATA_DIR / get_setting(
    "vectorstore.cloud_dir", "databases/vectorstore_cloud"
)
DATABASE_USER_RESULT_DIR_LOCAL = DATA_DIR / get_setting(
    "vectorstore.user_result_local_dir", "databases/user_result_local"
)
DATABASE_USER_RESULT_DIR_CLOUD = DATA_DIR / get_setting(
    "vectorstore.user_result_cloud_dir", "databases/user_result_cloud"
)
...
def get_collection_name():
    """根据嵌入模型类型获取集合名称"""
    prefix = get_setting(
        f"vectorstore.collection_prefix.{'cloud' if USE_CLOUD_EMBEDDING else 'local'}",
        "local_4b",
    )
